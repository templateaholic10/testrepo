...
double burgers(double v, double x, double t) { return sin(v-x)-v/t; }
double dburgers(double v, double x, double t) { return cos(v-x)-1/t; }
...
bool newton::step_forward() {
	...
	_pass.push_back(last - _f(last)/df(last));
	_step++;
...
int main(int argc, char const *argv[])
{
	...
	for (int i = 0; i < 1; ++i) // todo
	{
		...
		{
			f = bind(burgers, placeholders::_1, x, t[i]);
			df = bind(dburgers, placeholders::_1, x, t[i]);

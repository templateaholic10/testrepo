...
namespace EM {
    ...
    {
        for (int dist = 0; dist < mixture_num; dist++) {
            os << "pi^" << dist << ((dist != mixture_num - 1) ? delim : '\0');
        }
        ...
        for (int dist = 0; dist < mixture_num; dist++) {
            for (int j = 0; j < dim; j++) {
                os << "mu^" << dist << "_" << j << ((j != dim - 1) ? delim : '\0');
            }
            os << ((dist != mixture_num - 1) ? delim : '\0');
        }
        ...
        for (int dist = 0; dist < mixture_num; dist++) {
            for (int i = 0; i < dim; i++) {
                for (int j = 0; j < dim; j++) {
                    os << "sigma^" << dist << "_" << i << j << ((j != dim - 1) ? delim : '\0');
                }
                os << ((i != dim - 1) ? delim : '\0');
            }
            os << ((dist != mixture_num - 1) ? delim : '\0');
    ...
    void test_EM_gaussian_mixtures()
    {
        ...
        mus[0](0) = -50.; mus[0](1) = -50.;
        mus[1](0) = -50.; mus[1](1) = 50.;
        mus[2](0) = 50.; mus[2](1) = -50.;
        mus[3](0) = 50.; mus[3](1) = 50.;

        std::array <dmatrix <dim>, mixture_num> As;
        As[0](0, 0) = 10.; As[0](0, 1) = 0.;
        As[0](1, 0) = 0.; As[0](1, 1) = 10.;
        As[1](0, 0) = 10.; As[1](0, 1) = 0.;
        As[1](1, 0) = 0.; As[1](1, 1) = 10.;
        As[2](0, 0) = 10.; As[2](0, 1) = 0.;
        As[2](1, 0) = 0.; As[2](1, 1) = 10.;
        As[3](0, 0) = 10.; As[3](0, 1) = 0.;
        As[3](1, 0) = 0.; As[3](1, 1) = 10.;

        // 確率分布
        PD pd(pi, mus, As);

        // 出力
        fout.open("data01/square.trueparam");
        ...
        // 出力
        fout.open("data01/square.dataset");
    ...
    void test_EM_initial_value()
    {
        ...
        // データの読込
        std::ifstream fin("data01/square.dataset");
        DS            ds(fin, format);
        fin.close();

        // 出力ファイルの設定
        std::ofstream fout("data01/square.initialtest");
